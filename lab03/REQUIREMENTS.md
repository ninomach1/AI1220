...
- FR-14 [Must] The system shall let approved officers appeal a decision concerning their group's hidden event with a non-empty explanation, present available report evidence or event evidence captured under FR-13, together with decision history, to a moderator for review, and keep the event hidden until a moderator records a decision to restore it; an officer's direct restoration attempt shall be denied. [Source: UR-8]
...
- AC-2.3: Given students A and B currently RSVP'd and still permitted to view the event, while C withdrew, when the officer saves a new time or place, A and B each receive an in-service notice of old and new values within 60 seconds under A5, while C receives none. [FR-9, NFR-5]

- AC-2.4: Given a student still has an RSVP but has lost the membership required to view a members-only event, or the event is hidden, a correction creates only the generic FR-9 notice within NFR-5's deadline, with no event title, group, old/new values, or event link. If the student loses access after a detailed notice was created, opening it again shows the generic text. [FR-8, FR-9, FR-13, NFR-5]
...
- AC-5.2: Given an officer cancels an event at time T, new RSVPs are rejected; by T + 30 days, an inspection of all A6 storage locations finds no student-to-attendance records for that event, including withdrawn RSVPs, correction-notice recipient/event links and payloads, or attendance identifiers copied into moderation evidence. An unresolved appeal does not postpone deletion. [FR-16, NFR-1]
...
| S2: members-only events stay within the group | UR-5 | FR-8: audience checks on lists, links, and RSVP; FR-9: recheck access before displaying correction details | US-2, AC-2.2, AC-2.4 |
| S2/S3: tell RSVP'd students about corrections without exposing restricted or hidden event details | UR-6, UR-5, UR-16 | FR-9: check current access whenever displaying a notice; show old/new values if allowed, otherwise only generic text; NFR-5: proposed 60-second deadline | US-2, AC-2.3, AC-2.4 |
...
| S3: preserve evidence for a group's appeal | UR-8, UR-7 | FR-14: appeal and moderator-only restoration; FR-11/FR-13: evidence with or without a user report; FR-12: decision history | US-3, AC-3.3 |
...
- A9: Proposed resolution of correction notices versus event access: retain the duty to notify each current RSVP recipient, but disclose event details only while that recipient can view the event. After loss of required membership for a members-only event or event hiding, show only FR-9's generic text, including when reopening an earlier notice. This controls later displays; it cannot retract information already seen. S2/S3/S5 must confirm this policy.
...
- Q5 (S1/S2/S5): Are A2's officer access and optional name visibility acceptable, and does an in-service correction notice meet the need to tell students, or is a university email notice also required? Confirm A9's generic-notice rule when a recipient loses access to the event.
...
- Q7 (S5): Is every field and purpose in A3 necessary, and what retention/deletion periods apply to non-cancelled-event attendance, follows, other notifications, and account records? Cancelled-event correction-notice links remain subject to A6. If external attendance downloads are later requested, what policy governs copies outside the service?
